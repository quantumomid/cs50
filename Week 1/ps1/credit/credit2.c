// My solution plus ChatGPT for comments and function documentation!

// Include libraries
#include <cs50.h> // for the get_long function
#include <stdio.h> // for the printf function

// Declare function prototypes
/**
 * Prompts the user for a credit card number and ensures non-negative input.
 * 
 * @return long The credit card number provided by the user.
 */
long get_credit_card_number(void);

/**
 * Calculates the sum of the credit card number using Luhn's algorithm.
 * 
 * @param user_cc_number The credit card number provided by the user.
 * @return int The sum of the credit card number.
 */
int calculate_sum_of_cc_luhns(long user_cc_number);

/**
 * Determines the length of the credit card number.
 * 
 * @param user_cc_number The credit card number provided by the user.
 * @return int The length of the credit card number.
 */
int get_cc_length(long user_cc_number);

/**
 * Checks the validity and type of the credit card number.
 * 
 * @param is_valid A boolean indicating whether the credit card number is valid.
 * @param user_cc_number The credit card number provided by the user.
 * @param cc_length The length of the credit card number.
 * @return int A value indicating whether the credit card number is valid (1) or invalid (0).
 */
...
int main(void)
{
    long const user_credit_number = get_credit_card_number(); // get user credit card number input

    // Determine length of the credit card number
    int cc_length = get_cc_length(user_credit_number);

    // Calculate sum using Luhn's algorithm to determine if credit card number is valid
    ...
    // Determine and print the validity and type of credit card
    check_card_validity_and_type(is_valid, user_credit_number, cc_length);
}

// Prompts user for credit card number and returns it
long get_credit_card_number(void)
{
...
// Calculates the sum of the credit card digits using Luhn's algorithm and returns it
int calculate_sum_of_cc_luhns(long user_cc_number)
{
    ...
    while (cc > 0)
    {
        ...
        // If the current digit is one that should be doubled
        if (is_double == true)
        {
    ...
    // Determine the total sum of the credit card digits
    int total_sum = sum_of_doubled_numbers + sum_of_other_numbers;
    return total_sum;
}

// Returns the length of the credit card number
int get_cc_length(long user_cc_number)
{
    int cc_length = 0;
    long cc = user_cc_number;
