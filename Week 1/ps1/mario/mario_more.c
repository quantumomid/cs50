...
void print_grid(int size)
{
    ...
    {
        // print the appropiate spaces before '#' symbols so a right alligned pyramid is shown
        print_spaces(size - i);
        print_hashes(i + 1);

        // two empty spaces so we have an empty vertical space between the two pyramids
        printf("  ");

        // printing pyramids left alligned
        print_hashes(i + 1);
